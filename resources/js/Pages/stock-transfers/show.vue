...
defineOptions({
    layout: [DashboardLayout, { title: 'Detail Stock Transfer', panelId: 'stock-transfers' }],
});
...
const postRecord = () => router.post(`/stock-transfers/${record.value.id}/post`);
const cancelRecord = () => router.post(`/stock-transfers/${record.value.id}/cancel`);
</script>
...
<template>
    <div class="space-y-6">
        <div class="flex flex-col gap-4 border-b border-default pb-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <p class="text-sm text-muted">Stock Transfer</p>
                <h1 class="text-2xl font-bold text-highlighted">{{ record.reference_number }}</h1>
                <p class="text-sm text-muted">{{ record.from_store?.name || '-' }} ke {{ record.to_store?.name || '-' }}</p>
            </div>
        ...
        <div class="grid gap-4 md:grid-cols-3">
            ...
            <div class="rounded-lg border border-default bg-default p-4">
                <p class="text-xs font-medium uppercase text-muted">Posted</p>
                <p class="mt-1 text-sm font-semibold text-highlighted">{{ record.posted_by?.name || '-' }} {{ record.posted_at ? `· ${record.posted_at}` : '' }}</p>
            </div>
            <div class="rounded-lg border border-default bg-default p-4">
            ...
            <table class="min-w-full divide-y divide-default">
                ...
                <tbody class="divide-y divide-default">
                    <tr v-for="item in record.items || []" :key="item.id">
                        ...
                        <td class="px-4 py-3 text-sm text-muted">{{ item.from_warehouse?.name || '-' }} / {{ item.from_warehouse_location?.name || 'Tanpa lokasi' }}</td>
                        <td class="px-4 py-3 text-sm text-muted">{{ item.to_warehouse?.name || '-' }} / {{ item.to_warehouse_location?.name || 'Tanpa lokasi' }}</td>
