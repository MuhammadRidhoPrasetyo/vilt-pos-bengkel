    ...
    <div class="space-y-6">
        ...
        <form class="space-y-6" @submit.prevent="submit">
            ...
            <div class="space-y-3">
                ...
                <div v-for="(item, index) in form.items" :key="index" class="grid gap-3 rounded-lg border border-default bg-default p-4 lg:grid-cols-12 lg:items-start">
                    ...
                    <label class="grid gap-1 text-sm lg:col-span-2">
                        <span class="text-xs font-medium text-muted">Gudang</span>
                        <USelect v-model="item.warehouse_id" :items="warehouseOptions" class="w-full" />
                    </label>
                    <label class="grid gap-1 text-sm lg:col-span-2">
                        <span class="text-xs font-medium text-muted">Lokasi</span>
                    ...
                    <label class="grid gap-1 text-sm lg:col-span-1">
                        <span class="text-xs font-medium text-muted">Tipe</span>
                        <USelect v-model="item.adjustment_type" :items="[{ label: '+ Tambah', value: 'increase' }, { label: '- Kurang', value: 'decrease' }]" class="w-full" />
                    </label>
                    <label class="grid gap-1 text-sm lg:col-span-1">
                        <span class="text-xs font-medium text-muted">Qty</span>
                        <input v-model.number="item.quantity" type="number" min="1" class="w-full rounded-md border border-default bg-default px-2.5 py-1.5 text-center font-mono text-sm outline-none focus:border-primary" />
                    </label>
                    <label class="grid gap-1 text-sm lg:col-span-2">
                        <span class="text-xs font-medium text-muted">Unit Cost</span>
                        <div class="relative flex items-center">
                            <span class="absolute left-2.5 text-xs text-muted font-mono select-none">Rp</span>
                    ...
                    <div class="flex items-center justify-end lg:col-span-1 lg:pt-5">
                        <button type="button" class="flex size-9 items-center justify-center rounded-md border border-red-500/20 text-red-600 hover:bg-red-500/10 transition-colors" title="Hapus Item" @click="removeItem(index)">
                            <UIcon name="i-lucide-trash-2" class="size-4" />
                        </button>
                    </div>
                    <label class="grid gap-1 text-sm lg:col-span-12 mt-1">
                        <span class="text-xs font-medium text-muted">Catatan Item (Opsional)</span>
                        <input v-model="item.note" placeholder="Catatan khusus item ini..." class="w-full rounded-md border border-default bg-default px-3 py-1.5 text-xs outline-none focus:border-primary" />
                    </label>
                </div>
                <div v-if="form.items.length === 0" class="rounded-lg border border-dashed border-default bg-default p-8 text-center text-sm text-muted">
                    ...
                    <button type="button" class="mt-2 font-medium text-primary underline" @click="openProductModal">Pilih produk untuk adjustment</button>
                </div>
            ...
            <div class="flex max-h-[90vh] w-full max-w-4xl flex-col rounded-xl bg-default p-5 shadow-2xl">
                <div class="flex items-center justify-between border-b border-default pb-3">
                    <div>
                        ...
                        <p class="text-xs text-muted">Pilih satu atau beberapa barang untuk dimasukkan ke dokumen adjustment.</p>
                    </div>
