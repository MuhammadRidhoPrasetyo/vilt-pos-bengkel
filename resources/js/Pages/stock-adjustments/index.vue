...
defineOptions({
    layout: [DashboardLayout, { title: 'Stock Adjustment', panelId: 'stock-adjustments' }],
});
...
const applyFilters = () => router.get('/stock-adjustments', {
    search: search.value || undefined,
    status: status.value || undefined,
    store_id: storeId.value || undefined,
}, { preserveState: true, replace: true });

const postRecord = (record) => router.post(`/stock-adjustments/${record.id}/post`);
const cancelRecord = (record) => router.post(`/stock-adjustments/${record.id}/cancel`);
...
const confirmDelete = () => {
    if (deleteTarget.value) {
        router.delete(`/stock-adjustments/${deleteTarget.value.id}`);
    }
...
<template>
    <div class="space-y-6">
        <div class="flex flex-col gap-4 border-b border-default pb-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-2xl font-bold text-highlighted">Stock Adjustment</h1>
                <p class="text-sm text-muted">Dokumen koreksi stok dengan workflow draft, posted, dan cancelled.</p>
            </div>
            <Link href="/stock-adjustments/create" class="inline-flex items-center justify-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-inverted hover:bg-primary/90">
            ...
            <table class="min-w-full divide-y divide-default">
                <thead class="bg-elevated/40">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase text-muted">Toko</th>
                ...
                <tbody class="divide-y divide-default">
                    <tr v-for="record in rows" :key="record.id" class="hover:bg-elevated/20">
                        ...
                        <td class="px-4 py-3 text-sm text-muted">{{ record.store?.name || '-' }}</td>
                        <td class="px-4 py-3 text-sm text-muted">{{ record.occurred_at || '-' }}</td>
                        <td class="px-4 py-3 text-sm"><span class="rounded-full px-2.5 py-1 text-xs font-semibold" :class="badgeClass(record.status)">{{ record.status }}</span></td>
                        <td class="px-4 py-3 text-sm text-muted">{{ record.items_count || record.items?.length || 0 }}</td>
                        <td class="px-4 py-3">
                            <div class="flex justify-end gap-2">
                                <Link :href="`/stock-adjustments/${record.id}`" class="rounded-md border border-default p-2 hover:bg-elevated"><UIcon name="i-lucide-eye" class="size-4" /></Link>
                                <Link v-if="record.status === 'draft'" :href="`/stock-adjustments/${record.id}/edit`" class="rounded-md border border-default p-2 hover:bg-elevated"><UIcon name="i-lucide-pencil" class="size-4" /></Link>
                    ...
                    <tr v-if="rows.length === 0"><td colspan="6" class="px-4 py-8 text-center text-sm text-muted">Belum ada stock adjustment.</td></tr>
                </tbody>
            </table>
        </div>

        <DeleteConfirmationModal v-model:open="showDelete" title="Hapus Stock Adjustment" description="Draft stock adjustment akan dihapus permanen." @confirm="confirmDelete" />
    </div>
</template>
