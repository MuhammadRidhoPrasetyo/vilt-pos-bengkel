...
<template>
    <Head :title="title" />
    <div class="flex min-h-screen w-full flex-col bg-default text-highlighted antialiased font-sans lg:h-screen lg:overflow-hidden select-none">
        <!-- Top Workspace Bar -->
        <header class="sticky top-0 z-30 flex h-14 shrink-0 items-center justify-between border-b border-default bg-elevated/90 px-3 sm:px-4 backdrop-blur-md">
            ...
            <div class="flex items-center gap-2.5 sm:gap-3">
                <div class="flex size-8 sm:size-9 items-center justify-center rounded-lg bg-emerald-600 text-white shadow-lg shadow-emerald-600/20 shrink-0">
                    <UIcon name="i-lucide-shopping-cart" class="size-4 sm:size-5" />
                </div>
                <div class="min-w-0">
                    <h1 class="text-xs sm:text-sm font-bold tracking-tight text-highlighted flex items-center gap-1.5 truncate">
                        <span class="truncate">{{ title }}</span>
                        <span v-if="subtitle" class="hidden sm:inline-block rounded bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 px-2 py-0.5 text-xs font-mono font-bold">{{ subtitle }}</span>
                    </h1>
                    <p class="text-[10px] sm:text-[11px] text-muted truncate">Terminal Kasir POS & Pelunasan Servis Bengkel</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2 sm:gap-3 shrink-0">
                ...
                <div class="hidden md:flex items-center gap-1.5 rounded-lg border border-default bg-default px-2.5 py-1 font-mono text-xs font-semibold text-highlighted">
                    <UIcon name="i-lucide-clock" class="size-3.5 text-emerald-500" />
                    {{ currentTime }}
                </div>
                ...
                <button
                    type="button"
                    class="p-2 rounded-lg border border-default bg-default text-muted hover:text-highlighted hover:bg-elevated transition-colors"
